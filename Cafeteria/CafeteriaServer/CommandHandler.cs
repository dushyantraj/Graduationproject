...
public class CommandHandler
{
    ...
    private string HandleAddMenuItem(string[] parts, MySqlConnection connection)
    {
        if (parts.Length >= 8)
        {
            int priceIndex = parts.Length - 5;

            // Join item name correctly, removing quotes and trimming spaces
            string itemName = string.Join(" ", parts, 2, parts.Length - 7).Trim('"').Trim();
            ...
            var dto = new AddMenuItemDTO
            {
                ...
                CuisinePreference = parts[parts.Length - 2],
                SpiceLevel = parts[parts.Length - 3]
            };

            if (decimal.TryParse(parts[priceIndex], out decimal price))
            {
                if (int.TryParse(parts[priceIndex + 1], out int available) && (available == 0 || available == 1))
                {
                    dto.Price = price;
                    dto.Available = available;
                    //for debugging
                    //Console.WriteLine($"DTO: MenuType={dto.MenuType}, ItemName={dto.ItemName}, Price={dto.Price}, Available={dto.Available}, FoodType={dto.FoodType}, CuisinePreference={dto.CuisinePreference}, SpiceLevel={dto.SpiceLevel}");

                    string result = _menuOperations.AddMenuItemToMenu(connection, dto);

                    Console.WriteLine($"AddMenuItemToMenu result: {result}");
    ...
    {
        FeedbackService feedbackServiceFetch = new FeedbackService(connection);
    ...
    private static string HandleSendFeedbackForm(string[] parts, MySqlConnection connection)
    {
        if (parts.Length >= 2)
        {
            FeedbackService feedbackServiceForm = new FeedbackService(connection);
    ...
    private static string HandleSubmitFeedback(string[] parts, MySqlConnection connection)
    {
        ...
        if (match.Success)
        {
            FeedbackService feedbackServiceSubmit = new FeedbackService(connection);
    ...
    private static readonly DiscardMenu discardMenu = new DiscardMenu(_connection);
